...
alias mp4erase='shopt -s globstar; for f in **/*.mp4 ; do nice -n 10 ffmpeg -i "$f" -c:v libx265 -crf 40  -preset fast -vf "fps=5" -c:a aac -b:a 64k "${f}_temp"; mv -f "${f}_temp" "${f}"; done'
